			...
			<Container tw='bg-gray-100 -mx-8 -mt-8 pt-8 px-8'>
				<Content2Xl>
					...
					<HeroRow>
						<UpdateNotice>
							<UpdateNoticeIcon />
							Last updated on 20th April, 2021 - Added support for
							React v17 and TailwindCSS v2!
						</UpdateNotice>
						<TextColumn>
					...
					<SectionContainer id='landingPageDemos'>
						...
						<SectionDescription>
							We have {noOfLandingPages} premade landing pages.
							Click on the "View Live Demo" button to see them in
							action. Customizing or Creating your own custom
							landing page is really simple by using our UI
							components.
						</SectionDescription>
						<PreviewCards>
							{Object.entries(landingPages).map(
								([pageName, page], index) => (
					...
					<SectionContainer>
						...
						<SectionDescription>
							We also provide {noOfInnerPages} additional inner
							pages for your various needs like a signup, login,
							pricing, about us, contact, blog etc. To view them
							in action click the "View Live Demo" button.
						</SectionDescription>
						<PreviewCards>
							{Object.entries(innerPages).map(
								([pageName, page], index) => (
									<PreviewCardContainer key={index}>
										<PreviewCard
											initial='rest'
					...
					<SectionContainer id='componentDemos'>
						...
						<SectionDescription>
							We also provide {noOfComponentBlocks} components
							along with the premade landing pages so you can
							create your own landing page within minutes as you
							see fit. You can combine these components to create
							1000s of unique attractive web pages.
				...
				(block, index) =>
					lastVisibleBlockIndex + 1 >= index && (
						<Block
							...
							components={block}
							notifyIsVisible={() =>
								updateLastVisibleBlockIndex(index)
							}
						/>
					)
			)}
...
const Block = ({ notifyIsVisible, components }) => {
	...
	return (
		<div ref={ref} tw='mt-32'>
			...
			<Components>
				{Object.values(components.elements).map(
